...
var discountPrices = function (sentence, discount) {
  ...
  for (const temp of list) {
    const numTemp = temp.slice(1);
    if (temp.startsWith('$') && numTemp && !pattern.test(numTemp)) {
      const num = Number(numTemp);
      if (num || num === 0) {
        const price = num * (1 - discount / 100);
        list[list.indexOf(temp)] = `$${price.toFixed(2)}`;
      }
    }
  }
  return list.join(' ');
